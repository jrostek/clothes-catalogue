...
// and transaction start and commit; Redis adds a span per cache command. Pino
// records are exported as OTLP logs carrying the trace and span id of the
// request that emitted them. The SDK is configured entirely from the OTEL_*
// environment variables that the Aspire AppHost injects (endpoint, protocol,
// headers, service name, resource attributes). Without an OTLP endpoint — e.g. a plain `nest:start:dev` —
